...
from tensorflow.keras.layers import Dense, Dropout, Conv2D, LayerNormalization,UpSampling2D
...
class Pyramid3DDecoder(tf.keras.Model):
    def __init__(self,config,img_size,use_pyramid=False,model_name='PyrDecoder',split_pred=False,
        ...
        if flow_sep_decode:
            ...
            if sep_conv:
                self.upconv_f = [
                tf.keras.layers.ConvLSTM2D(filters=96,activation='swish',name=f'upconvf_{1}_0',return_sequences=True,**conv2d_kwargs),
                tf.keras.layers.Conv2D(filters=48,activation='swish',name=f'upconvf_{0}_0',**conv2d_kwargs)
                ]
            else:
                self.upconv_f = [
                    tf.keras.layers.Conv2D(
...
class STrajNet(tf.keras.Model):
    def __init__(self,cfg,model_name='STrajNet',use_pyramid=True,actor_only=True,sep_actors=False,
        ...
        self.encoder = SwinTransformerEncoder(include_top=True,img_size=cfg['input_size'], window_size=cfg[
            ...
            sep_encode=True,flow_sep=True,use_flow=True,drop_rate=0.5, attn_drop_rate=0.1,drop_path_rate=0.1,
        ...
        self.fg = fg
        if fg_msa:
            self.fg_msa_layer = FGMSA(q_size=(16,16), kv_size=(16,16), n_heads=8, n_head_channels=48,n_groups=8,in_dim=384,out_dim=384,attn_drop=0.125, proj_drop=0.125, fg=True)
        self.decoder = Pyramid3DDecoder(config=None,img_size=cfg['input_size'],use_pyramid=use_pyramid,timestep_split=True,
        shallow_decode=(4-len(cfg['depths'][:])),flow_sep_decode=True,conv_cnn=True,sep_conv=True)
