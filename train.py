...
parser.add_argument('--save_dir', type=str, help='saving directory',default="/raid/STrajNet/train_output")
parser.add_argument('--file_dir', type=str, help='Training Val Dataset directory',default="/raid/STrajNet/preprocessed_data")
parser.add_argument('--model_path', type=str, help='loaded weight path',default='/raid/STrajNet/train_output/model_15_37.3039_37.6265.tf')
parser.add_argument('--batch_size', type=int, help='batch_size',default=2)
parser.add_argument('--epochs', type=int, help='training eps',default=16)
parser.add_argument('--lr', type=float, help='initial learning rate',default=9e-5)
...
with strategy.scope():
    ...
    train_metrics = OGMFlowMetrics(prefix='train')
    valid_metrics = OGMFlowMetrics(prefix='val')
...
with strategy.scope():
    ...
    optimizer = tf.keras.optimizers.Nadam(learning_rate=LR, clipnorm=1.0)  # added clipnorm for gradient clipping
...
def train_step(data):
    ...
    prev_weights=model.trainable_weights[:]
    optimizer.apply_gradients(zip(grads, model.trainable_weights))
    print("Weight Change status after applying gradients", prev_weights==model.trainable_weights)
    tf.debugging.check_numerics(outputs, 'Model outputs have NaN or Inf values')
...
def model_training(train_dataset, valid_dataset, epochs,continue_ep=16):
    ...
    for epoch in range(epochs):
        ...
        try :  
            ...
            exception_counter += 1
            print(f"{e}")
            print("corrupt file counter per epoch, count= ",exception_counter)
    ...
    model.save_weights(f'{SAVE_DIR}/final_model_new.tf')
