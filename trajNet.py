...
class MapEncoder(tf.keras.layers.Layer):
    def __init__(self,num_heads=4,out_dim=256):
        ...
        self.node_feature = tf.keras.layers.Conv1D(64, 1,activation='swish')
        ...
        self.sublayer = tf.keras.layers.Dense(out_dim, activation='swish')
...
class TrajEncoderLSTM(tf.keras.layers.Layer):
    def __init__(self,out_dim=256):
        ...
        self.embed = tf.keras.layers.Conv1D(64, 1, activation='swish')
...
class Cross_Attention(tf.keras.layers.Layer):
    def __init__(self, num_heads, key_dim,conv_attn=False):
        ...
        self.FFN1 = tf.keras.layers.Dense(4*key_dim, activation='swish')
...
#         embed = tf.repeat([self.bi_embed], repeats=tf.shape(occ)[0],axis=0)
...
#         c_attn_mask = tf.cast(c_attn_mask, tf.float16)
...
#         return obs,occ,c_attn_mask
class TrajNet(tf.keras.Model):
    def __init__(self, cfg, past_to_current_steps=11, obs_actors=48, occ_actors=16, actor_only=True, no_attn=False, double_net=False):
        super(TrajNet, self).__init__()
        ...
        if not no_attn:
            if double_net:
                self.cross_attention = [Cross_Attention(num_heads=cfg['att_heads'], key_dim=192) for _ in range(2)]
            else:
                self.cross_attention = Cross_Attention(num_heads=cfg['att_heads'], key_dim=cfg['out_dim'])
        ...
        dummy_obs_actors = tf.zeros([1, obs_actors, past_to_current_steps, 8], dtype=tf.float16)
        dummy_occ_actors = tf.zeros([1, occ_actors, past_to_current_steps, 8], dtype=tf.float16)

        self.bi_embed = tf.cast(tf.repeat([[1, 0], [0, 1]], repeats=[obs_actors, occ_actors], axis=0), tf.float16)
        self.seg_embed = tf.keras.layers.Dense(cfg['out_dim'], use_bias=False)

        self(dummy_obs_actors, dummy_occ_actors)

    def call(self, obs_traj, occ_traj, map_traj=None, training=True):
        obs_traj = tf.cast(obs_traj, dtype=tf.float16)
        occ_traj = tf.cast(occ_traj, dtype=tf.float16)

        obs_mask = tf.cast(tf.not_equal(obs_traj, 0)[:, :, :, 0], dtype=tf.float16)
        obs = [self.traj_encoder(obs_traj[:, i], obs_mask[:, i], training) for i in range(self.obs_actors)]
        obs = tf.stack(obs, axis=1)

        occ_mask = tf.cast(tf.not_equal(occ_traj, 0)[:, :, :, 0], dtype=tf.float16)
        occ = [self.traj_encoder(occ_traj[:, i], occ_mask[:, i], training) for i in range(self.occ_actors)]
        occ = tf.stack(occ, axis=1)
        ...
        c_attn_mask = tf.cast(tf.not_equal(tf.reduce_sum(tf.cast(tf.concat([obs_mask, occ_mask], axis=1), tf.float16), axis=-1), 0), dtype=tf.float16)
        ...
        attn_mask = tf.matmul(c_attn_mask[:, :, tf.newaxis], c_attn_mask[:, tf.newaxis, :])
...
class Cross_AttentionT(tf.keras.layers.Layer):
    def __init__(self, num_heads, key_dim,output_dim,conv_attn=False,sep_actors=False):
        ...
        if sep_actors:
            ...
            self.aFFN1 = tf.keras.layers.Dense(4*key_dim, activation='swish')
...
class TrajNetCrossAttention(tf.keras.Model):
    ...
    def call(self,pic_encode,obs_traj,occ_traj,map_traj=None,training=True,flow_pic_encode=None):
        ...
        pic_mask = tf.ones_like(flat_encode[:,0,:,0],tf.float16)
